...
public class PersonaREST {
	...
	private ResponseEntity<Persona> guardar (@RequestBody Persona persona){
		...
		try {
			return ResponseEntity.created(new URI("/api/persona"+temporal.getIdentificacion())).body(temporal);
	...
	private ResponseEntity<Optional<Persona>> listarPersonasPorID (@PathVariable ("identificacion") Long identificacion){
		return ResponseEntity.ok(personaService.findById(identificacion));
	}
